odoo.define('product_qty.quantity', function (require) {
'use strict';

var websiteSaleCart = require('website_sale.cart');
...
publicWidget.registry.WebsiteSale = publicWidget.Widget.extend(VariantMixin, {
    ...
    _changeCartQuantity: function ($input, value, $dom_optional, line_id, productIDs) {
        console.log("_changeCartQuantity=============eeeeeeeeeeee 111======",value)
    ...
    _onChangeCartQuantity: function (ev) {
        ...
        console.log("value====eeeeeeeeee======",value)
        if (isNaN(value)) {
            value = 1;
        }
...
console.log("thank you=====================")
    if($('.css_quantity')){
    $('.js_product .css_quantity').addClass('d-none')
  }

  $('#select_uom').on('change',function(){
        var val = $('#select_uom').val()
        var product_price = parseFloat($('#product_price').val())

        if(val == '250'){
            $('#select_sub_uom').addClass('d-none')
            $('#need_more').addClass('d-none')
            $('.need_weight').addClass('d-none')
            document.getElementById("add_to_cart").disabled = false;
            $('.quantity').val(0.25)
            $('.oe_currency_value').html(product_price*0.25)
            $('#uom_dscription').html("You are buying: 250 grams")
        }
        else if(val == '500'){
            $('#select_sub_uom').addClass('d-none')
            $('#need_more').addClass('d-none')
            ...
            document.getElementById("add_to_cart").disabled = false;
            $('.quantity').val(0.5)
            $('.oe_currency_value').html(product_price*0.5)
            $('#uom_dscription').html("You are buying: 500 grams")
        }
        else if(val == '1'){
            $('.quantity').val(1)
            $('.oe_currency_value').html(product_price)
             $('#select_sub_uom').removeClass('d-none')
             $('#need_more').addClass('d-none')
             $('.add_weight').removeClass('d-none')
             $('.need_weight').addClass('d-none')
             document.getElementById("add_to_cart").disabled = false;
             $('#select_sub_uom').val(0)
             $('#uom_dscription').html("You are buying: 1 KG")
        }
        else if(val == '2'){
            $('.quantity').val(2)
            $('.oe_currency_value').html(product_price*2)
             $('#select_sub_uom').removeClass('d-none')
             $('#need_more').addClass('d-none')
             $('#select_sub_uom').val(0)
             $('.add_weight').removeClass('d-none')
             $('.need_weight').addClass('d-none')
             document.getElementById("add_to_cart").disabled = false;
             $('#uom_dscription').html("You are buying: 2 KG")
        }
        else if(val == '3'){
            $('.quantity').val(3)
            $('.oe_currency_value').html(product_price*3)
             $('.select_sub_uom').removeClass('d-none')
             $('#need_more').addClass('d-none')
             $('#select_sub_uom').val(0)
             $('.add_weight').removeClass('d-none')
             $('.need_weight').addClass('d-none')
             document.getElementById("add_to_cart").disabled = false;
             $('#uom_dscription').html("You are buying: 3 KG")
        }
        else if(val == '4'){
            $('.quantity').val(4)
            $('.oe_currency_value').html(product_price*4)
             $('#select_sub_uom').removeClass('d-none')
             $('#need_more').addClass('d-none')
             $('#select_sub_uom').val(0)
             $('.add_weight').removeClass('d-none')
             $('.need_weight').addClass('d-none')
             document.getElementById("add_to_cart").disabled = false;
             $('#uom_dscription').html("You are buying: 4 KG")
        }
        else if(val == '5'){
            $('.quantity').val(5)
            $('.oe_currency_value').html(product_price*5)
             $('#select_sub_uom').removeClass('d-none')
             $('#need_more').addClass('d-none')
             $('#select_sub_uom').val(0)
             $('.add_weight').removeClass('d-none')
             $('.need_weight').addClass('d-none')
             document.getElementById("add_to_cart").disabled = false;
             $('#uom_dscription').html("You are buying: 5 KG")
        }
        else if(val == '6'){
            $('.quantity').val(6)
            $('.oe_currency_value').html(product_price*6)
             $('#select_sub_uom').removeClass('d-none')
             $('#need_more').addClass('d-none')
             $('#select_sub_uom').val(0)
             $('.add_weight').removeClass('d-none')
             $('.need_weight').addClass('d-none')
             document.getElementById("add_to_cart").disabled = false;
             $('#uom_dscription').html("You are buying: 6 KG")
        }
        else if(val == '7'){
            $('.quantity').val(7)
            $('.oe_currency_value').html(product_price*7)
             $('#select_sub_uom').removeClass('d-none')
             $('#need_more').addClass('d-none')
             $('#select_sub_uom').val(0)
             $('.add_weight').removeClass('d-none')
             $('.need_weight').addClass('d-none')
             document.getElementById("add_to_cart").disabled = false;
             $('#uom_dscription').html("You are buying: 7 KG")
        }
        else if(val == '8'){
            $('.quantity').val(8)
            $('.oe_currency_value').html(product_price*8)
             $('#select_sub_uom').removeClass('d-none')
             $('#need_more').addClass('d-none')
             $('#select_sub_uom').val(0)
             $('.add_weight').removeClass('d-none')
             $('.need_weight').addClass('d-none')
             document.getElementById("add_to_cart").disabled = false;
             $('#uom_dscription').html("You are buying: 8 KG")
        }
        else if(val == '9'){
            $('.quantity').val(9)
            $('.oe_currency_value').html(product_price*9)
             $('#select_sub_uom').removeClass('d-none')
             $('#need_more').addClass('d-none')
             $('#select_sub_uom').val(0)
             $('.add_weight').removeClass('d-none')
             $('.need_weight').addClass('d-none')
             $('#uom_dscription').html("You are buying: 9 KG")
        }
        else if(val == '10'){
            $('.quantity').val(10)
            $('.oe_currency_value').html(product_price*10)
             $('#select_sub_uom').removeClass('d-none')
             $('#need_more').addClass('d-none')
             $('#select_sub_uom').val(0)
             $('.add_weight').removeClass('d-none')
             $('.need_weight').addClass('d-none')
             document.getElementById("add_to_cart").disabled = false;
             $('#uom_dscription').html("You are buying: 10 KG")
        }
        else if(val == '11'){
//        $('.oe_currency_value').html(product_price*11)
           $('#select_sub_uom').addClass('d-none')
           $('#need_more').removeClass('d-none')
           $('.need_weight').removeClass('d-none')
           $('.add_weight').addClass('d-none')
  ...
  $('#select_sub_uom').on('change',function(){
        var val = parseFloat($('#select_uom').val())
        var sub_val = parseFloat($('#select_sub_uom').val())
        var total = val + sub_val
        var text_string = "You are buying: "+total+" KG"
        var product_price = parseFloat($('#product_price').val())

       if(sub_val){
            $('.quantity').val(total)
            $('.oe_currency_value').html(product_price*total)
            $('#uom_dscription').html(text_string)
       }
       else{
        $('.quantity').val(val)
        $('.oe_currency_value').html(product_price*val)
        $('#uom_dscription').html(text_string)
       }
       })

  $('#need_more').on('change',function(){
            var val = parseFloat($('#need_more').val())
            var product_price = parseFloat($('#product_price').val())
            var text_string = "You are buying: "+val+" KG"
            if(val>=11){
            $("#add_to_cart").removeClass("disabled");
                document.getElementById("add_to_cart").disabled = false;
                $('.error_need').addClass('d-none')
                $('.quantity').val(val)
                $('.oe_currency_value').html(product_price*val)
                $('#uom_dscription').html(text_string)
            }
